...
const {cohorts, students, instructors} = require('../db');
        ...
        cohorts
          .find({
            ...
            if(cohort && cohort.length > 0) {
              ...
              return fetchJSON(getPerformanceURL(cohort_id), getAuthHeader())
                .then(data => {
                  ...
                  formatPerformanceData(data)
                    .then(() => {
                      return Promise.all([
                        Promise.all(data.students.map(student => {
                          return students
                        ...
                        Promise.all(data.instructors.map(instructor => {
                          return instructors
...
function getPerformanceURL(cohort_id) {
  return `https://learn.galvanize.com/api/v1/cohorts/${cohort_id}/performances`;
}
